...
namespace BLEPP
{
	...
	const ServiceInfo* lookup_service_by_UUID(const UUID& uuid)
	{
		...
		if(vec.size() == 0)
		{
			...
			add( "Environmental Sensing",         "org.bluetooth.service.environmental_sensing",     0x181A);
	...
	int log_l2cap_options(int sock)
	{
		...
		LOGVAR(Debug,options.omtu);
		LOGVAR(Debug,options.imtu);
		LOGVAR(Debug,options.flush_to);
		LOGVAR(Debug,options.mode);
		LOGVAR(Debug,options.fcs);
		LOGVAR(Debug,options.max_tx);
		LOGVAR(Debug,options.txwin_size);
    ...
    void BLEGATTStateMachine::connect_blocking_randaddr(const string& address)
    {
        connect(address, true, false);
    }

    void BLEGATTStateMachine::connect_nonblocking_randaddr(const string& address)
    {
        connect(address, false, false);
    }
	...
	void BLEGATTStateMachine::connect(const string& address, bool blocking, bool pubaddr, string device)
	{
		...
		if (device == "") sba.l2_bdaddr={{0,0,0,0,0,0}}; //use default adapter
			...
			bdaddr_t btsrc_addr;
			dev_id = hci_devid(device.c_str()); //obtain device id from HCI device name
        ...
        int bret;
		log_fd(bret = bind(sock, (sockaddr*)&sba , sizeof(sba)));

		if (bret != 0)
        {
		    LOG(Debug, "Socket Bind Failed: " << strerror(errno));
            close_and_cleanup();
            cb_disconnected(Disconnect(Disconnect::Reason::ConnectionFailed, errno));
        } else {
            LOG(Debug, "Socket Bind Succeeded");
        }
		...
		LOG(Debug, "str2ba = " << rr);
		int ret;
		log_fd(ret = ::connect(sock, (sockaddr*)&addr, sizeof(addr)));
		...
		else if(errno == ENOSYS) // Restart to handle "Function not implemented"
        {
            close_and_cleanup();
            cb_disconnected(Disconnect(Disconnect::Reason::ConnectionFailed, errno));
        }
