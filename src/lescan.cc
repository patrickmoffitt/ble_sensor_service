...
namespace BLEPP
{
	...
	void HCIScanner::start()
	{
		...
		LOG(LogLevels::Debug, "Starting scanner");
	...
	void HCIScanner::stop()
	{
		...
		LOG(LogLevels::Debug, "Cleaning up HCI scanner");
	...
	*/
	...
	vector<AdvertisingResponse> parse_event_packet(Span packet)
	{
		...
		{
			LOG(Debug, "event_code = 0x" << hex << (int)event_code << ": Meta event" << dec);
			LOGVAR(Debug, length);
		...
		{
			LOG(Debug, "event_code = 0x" << hex << (int)event_code << dec);
			LOGVAR(Debug, length);
	...
	vector<AdvertisingResponse> parse_le_meta_event(Span packet)
	{
		...
		{
			LOG(Debug, "subevent_code = 0x02: LE Advertising Report Event");
			return parse_le_meta_event_advertisement(packet);
		}
		else
		{
			LOGVAR(Debug, subevent_code);
			return {};
		}
	}
	...
	vector<AdvertisingResponse> parse_le_meta_event_advertisement(Span packet)
	{
		...
		for(int i=0; i < num_reports; i++)
		{
			...
			if(event_type == LeAdvertisingEventType::ADV_IND)
				LOG(Debug, "event_type = 0x00 ADV_IND, Connectable undirected advertising");
			else if(event_type == LeAdvertisingEventType::ADV_DIRECT_IND)
				LOG(Debug, "event_type = 0x01 ADV_DIRECT_IND, Connectable directed advertising");
			else if(event_type == LeAdvertisingEventType::ADV_SCAN_IND)
				LOG(Debug, "event_type = 0x02 ADV_SCAN_IND, Scannable undirected advertising");
			else if(event_type == LeAdvertisingEventType::ADV_NONCONN_IND)
				LOG(Debug, "event_type = 0x03 ADV_NONCONN_IND, Non connectable undirected advertising");
			else if(event_type == LeAdvertisingEventType::SCAN_RSP)
				LOG(Debug, "event_type = 0x04 SCAN_RSP, Scan response");
			else
			...
			if(address_type == 0)
				LOG(Debug, "Address type = 0: Public device address");
			else if(address_type == 1)
				LOG(Debug, "Address type = 0: Random device address");
			else
				LOG(Debug, "Address type = 0x" << to_hex(address_type) << ": unknown");
			...
			if(rssi == 127)
				LOG(Debug, "RSSI = 127: unavailable");
			else if(rssi <= 20)
				LOG(Debug, "RSSI = " << (int) rssi << " dBm");
			else
				LOG(Debug, "RSSI = " << to_hex((uint8_t)rssi) << " unknown");
			...
			try{
				...
				while(data.size() > 0)
				{
					...
					if(type == GAP::flags)
					{
						...
						LOG(Debug, "Flags = " << to_hex(rsp.flags->flag_data));

						if(rsp.flags->LE_limited_discoverable)
							LOG(Debug, "        LE limited discoverable");

						if(rsp.flags->LE_general_discoverable)
							LOG(Debug, "        LE general discoverable");

						if(rsp.flags->BR_EDR_unsupported)
							LOG(Debug, "        BR/EDR unsupported");

						if(rsp.flags->simultaneous_LE_BR_host)
							LOG(Debug, "        simultaneous LE BR host");

						if(rsp.flags->simultaneous_LE_BR_controller)
							LOG(Debug, "        simultaneous LE BR controller");
					}
